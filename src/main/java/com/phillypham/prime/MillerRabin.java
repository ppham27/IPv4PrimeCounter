...
public class MillerRabin
    ...
    public boolean isProbablePrime(long p) {
        return isProbablePrime(p, 20);
    }

    public boolean isProbablePrime(long p, int n) {
        if (p < 2) return false;        
    ...
    private long modExp(long a, long e, long m) {
        if (m*m >= 0) {         // m is small
            long res = 1;
            while (e > 0) {
    ...
    private boolean witnessesPrime(long witness, long p, long s, long d) {
